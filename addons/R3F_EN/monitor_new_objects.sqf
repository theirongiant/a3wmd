...
_liste_objets_depl_heli_remorq_transp = R3F_LOG_CFG_moveable_objects + R3F_LOG_CFG_liftable_objects + R3F_LOG_CFG_towable_objects + R3F_LOG_transportable_object_classes;
#endif
		...
		if (_count_liste_vehicules > 0) then
		{
			...
			// On parcoure tout les véhicules présents dans le jeu en 18 secondes
			{
				...
				if !(_objet getVariable ["R3F_LOG_init_done", false]) then
				{
					...
					{
						[_objet] spawn R3F_LOG_FNC_object_init;
					};
					
					// If vehicle can airlift
					if ({_objet isKindOf _x} count R3F_LOG_CFG_helicopters > 0) then
					{
						[_objet] spawn R3F_LOG_FNC_helicopter_init;
					};

					// If vehicle can transport contents
					if ({_objet isKindOf _x} count R3F_LOG_transport_classes > 0) then
					{
						[_objet] spawn R3F_LOG_CFG_transporter_init;
					};
					
					// If vehicle can tow
					if ({_objet isKindOf _x} count R3F_LOG_CFG_towers > 0) then
					{
						[_objet] spawn R3F_LOG_FNCT_tower_init;
					};
