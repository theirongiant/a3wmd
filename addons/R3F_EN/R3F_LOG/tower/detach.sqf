...
else
{
	...
	private ["_tower", "_objet"];
	
	_objet = _this select 0;
	_tower = _objet getVariable "R3F_LOG_est_transporte_par";
	
	// Ne pas permettre de décrocher un objet s'il est porté héliporté
	if ({_tower isKindOf _x} count R3F_LOG_CFG_towers > 0) then
	{
		...
		_tower setVariable ["R3F_LOG_remorque", objNull, true];
		...
		if ({_objet isKindOf _x} count R3F_LOG_CFG_moveable_objects > 0) then
		{
			...
			if (isNull (_tower getVariable "R3F_LOG_remorque") &&
		...
		{
			player globalChat STR_R3F_LOG_action_tow_detach_object_confirmation;
		};
	}
	else
	{
		player globalChat STR_R3F_LOG_action_tow_only_pilot_can_detach;
	};
