/**
 * Remorque l'objet déplacé par le joueur avec un tower
 * 
...
else
{
	...
	_tower = nearestObjects [_objet, R3F_LOG_CFG_towers, 22];
	// Parce que le tower peut être un objet remorquable
	_tower = _tower - [_objet];
	
	if (count _tower > 0) then
	{
		_tower = _tower select 0;
		
		if (alive _tower && isNull (_tower getVariable "R3F_LOG_remorque") && ((velocity _tower) call BIS_fnc_magnitude < 6) && (getPos _tower select 2 < 2) && !(_tower getVariable "R3F_LOG_disabled")) then
		{
