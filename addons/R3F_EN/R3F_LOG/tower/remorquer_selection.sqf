/**
 * Remorque l'objet sélectionné (R3F_LOG_selected_object) à un véhicule
 * 
 * @param 0 le tower
 * 
...
else
{
	...
	if (!(isNull _objet) && (alive _objet) && !(_objet getVariable "R3F_LOG_disabled")) then
	{
		...
		{
			if (_objet distance _tower <= 30) then
			{
             	//The vehicle that is driving.   
                _tempobj = _tower;		_countTransportedBy = 1;
                ...
                if(_countTransportedBy + _countTowedVehicles <= 2) then
                {
					...
					_tower setVariable ["R3F_LOG_remorque", _objet, true];
					// On mémorise aussi sur le réseau que le canon est attaché en remorque
					_objet setVariable ["R3F_LOG_est_transporte_par", _tower, true];
					...
					_towerBB = _tower call fn_boundingBoxReal;
					...
					_towerGroundPos = _tower worldToModel (_tower call fn_getPos3D);
			...
			{
				player globalChat format [STR_R3F_LOG_action_tow_too_far_from_tower, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
			};
		}
		else
		{
			player globalChat format [STR_R3F_LOG_action_tow_object_in_transit, getText (configFile >> "CfgVehicles" >> (typeOf _objet) >> "displayName")];
		};
	};
