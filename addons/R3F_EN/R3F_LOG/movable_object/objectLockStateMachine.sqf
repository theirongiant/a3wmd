...
if(R3F_LOG_mutex_local_lock) exitWith {
...
switch (_lockState) do
{
    ...
    if !(R3F_LOG_mutex_local_lock) then {
        R3F_LOG_mutex_local_lock = false;
        diag_log format["WASTELAND DEBUG: An error has occured in LockStateMachine.sqf. Mutex lock was not reset. Mutex lock state actual: %1", R3F_LOG_mutex_local_lock];
    }; 
};
