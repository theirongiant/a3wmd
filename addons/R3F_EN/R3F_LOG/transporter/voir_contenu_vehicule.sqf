...
else
{
	...
	uiNamespace setVariable ["R3F_LOG_dlg_CV_transporter", _transporter];
	...
	for [{_i = 0}, {_i < count _contenu}, {_i = _i + 1}] do
	{
		...
		for [{_j = 0}, {_j < count R3F_LOG_CFG_transportable_objects}, {_j = _j + 1}] do
		{
			if (_objet isKindOf (R3F_LOG_CFG_transportable_objects select _j select 0)) exitWith
			{
				_chargement_actuel = _chargement_actuel + (R3F_LOG_CFG_transportable_objects select _j select 1);
			};
		};
	};
	...
	(_dlg_contenu_vehicule displayCtrl R3F_LOG_IDC_dlg_CV_titre) ctrlSetText STR_R3F_LOG_dlg_CV_title;
	(_dlg_contenu_vehicule displayCtrl R3F_LOG_IDC_dlg_CV_credits) ctrlSetText STR_R3F_ARTY_LOG_nom_produit;
	(_dlg_contenu_vehicule displayCtrl R3F_LOG_IDC_dlg_CV_btn_decharger) ctrlSetText STR_R3F_LOG_dlg_CV_btn_unload;
	(_dlg_contenu_vehicule displayCtrl R3F_LOG_IDC_dlg_CV_btn_fermer) ctrlSetText STR_R3F_LOG_dlg_CV_btn_cancel;
	/**** FIN des traductions des labels ****/
	
	(_dlg_contenu_vehicule displayCtrl R3F_LOG_IDC_dlg_CV_capacite_vehicule) ctrlSetText (format [STR_R3F_LOG_dlg_CV_vehicle_capacity, _chargement_actuel, _chargement_maxi]);
	...
	else
	{
		...
		for [{_i = 0}, {_i < count _tab_objets}, {_i = _i + 1}] do
		{
			...
			else
			{
				...
				{
					_index = _ctrl_liste lbAdd ("     " + STR_R3F_LOG_arti_name+ format [" (%1x)", _tab_quantite select _i]);
				}
				else
				{
					_index = _ctrl_liste lbAdd ("     " + getText (configFile >> "CfgVehicles" >> (_tab_objets select _i) >> "displayName") + format [" (%1x)", _tab_quantite select _i]);
