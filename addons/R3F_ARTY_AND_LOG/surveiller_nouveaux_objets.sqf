 ...
 * 
 * GT: Periodically checks for new items to add their functionality artillery and logistics if required 
 * Script to run in a thread dedicated execution
 *
...
// GT: wait for briefing to finish
sleep 0.1;
...
//MD-  Concatenate arrays of objects used in isKindOf
...
// GT: Contain the list of vehicles (and objects) already initialized 
// GT: vehicles known list = [];

while {true} do
{
	if !(isNull player) then //MD- player is active
	{
		...
		// GT: Retrieving all new vehicles and new map objects derived from "Static" (box of ammo, flag, ...) near the player
		...
		{
			//MD- Sleeps a variable about depending on number of found items
			//MD- 10 seconds for 1, 1 second for 100
			_sleepDelay = 10 / _count_liste_vehicules;
			
			// On parcoure tout les véhicules présents dans le jeu en 18 secondes
			// GT: It goes through all the vehicles in the game in 18 seconds
			{
				_objet = _x;
				
				//MD- Ignore if it's already been initialised
				//MD- second variable is a  default value to return if the value is null
				if !(_objet getVariable ["R3F_LOG_init_done", false]) then
				{
						...
						//MD- set init_done to true, note this isn't broadcast
			...
			// GT: Objects have been initialized, it stores them for not re-initialize
			{
