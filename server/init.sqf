...
//MD- Run all these scripts, waiting for the serverCompile to finish before continuing
//MD- I'll come back and look  at these later
...
//MD- If serverRules is not empty
if (loadFile (externalConfigFolder + "\serverRules.sqf") != "") then
{
	//MD- Fuck knows seems to be some sort of anti-hack measure?
	//MD- Okay lets break this down
	//MD- [call compile preprocessFileLineNumbers (externalConfigFolder + "\serverRules.sqf")]
	//MD- run this script and use the return value in an array
	//MD- [[retVal, "client\functions\defineServerRules.sqf], "BIS_fnc_execVM", true, true]
	//MD- and pass it to TPG_fnc_MP
	//MD- TPG_fnc_MP is declared a few places but serverCompile (called above) looks likely
	//MD- Although following through antihack leads to it being defined (compileFuncs) while in 
	//MD- serverCompile it only set it if it's null, so that looks like our boy.
